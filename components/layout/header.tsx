...
const disciplines = [
  ...
  { name: "Commuter", href: "/bikes?discipline=commuter" },
];
...
export function Header() {
  ...
  return (
    <header className="fixed top-0 w-full z-50" style={{ background: "rgba(10,10,10,0.92)", backdropFilter: "blur(16px) saturate(180%)", borderBottom: "1px solid rgba(245,240,235,0.06)" }}>
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <nav className="hidden lg:flex items-center gap-1">
            <div className="relative group" onMouseEnter={() => setBikesOpen(true)} onMouseLeave={() => setBikesOpen(false)}>
              <Link href="/bikes" className="px-3 py-2 text-sm font-medium transition-colors" style={{ color: "rgba(245,240,235,0.7)" }}>
                Bikes
              </Link>
              {bikesOpen && (
            ...
            {[{ label: "Service", href: "/service" }, { label: "About", href: "/about" }, { label: "Contact", href: "/contact" }].map((item) => (
              <Link key={item.href} href={item.href} className="px-3 py-2 text-sm font-medium transition-colors hover:text-white" style={{ color: "rgba(245,240,235,0.7)" }}>
                {item.label}
              </Link>
            ))}
          </nav>
          ...
          <div className="flex items-center gap-3">
            ...
            <button className="p-2 transition-colors hover:text-white" style={{ color: "rgba(245,240,235,0.5)" }} aria-label="Cart"><ShoppingBag className="w-5 h-5" /></button>
      ...
      {open && (
        <div className="lg:hidden px-6 py-4 space-y-2" style={{ background: "rgba(10,10,10,0.98)", borderTop: "1px solid rgba(245,240,235,0.06)" }}>
          {[{ label: "Bikes", href: "/bikes" }, { label: "Service", href: "/service" }, { label: "About", href: "/about" }, { label: "Contact", href: "/contact" }, { label: "Sale", href: "/sale" }].map((item) => (
